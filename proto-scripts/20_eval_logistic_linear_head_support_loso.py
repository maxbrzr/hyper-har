...
class Config:
    ...
    eval_stage_name: str = "20_logistic_linear_head_support_loso"
...
def _logistic_episode_predict(
    ...
    if not support_indices:
        raise ValueError("Logistic linear-head fit needs support embeddings.")
    if float(config.inverse_regularization) <= 0:
        raise ValueError("inverse_regularization must be > 0.")
    ...
    support_x = support_emb.numpy()
    query_x = query_emb.numpy()
    y = np.asarray(support_y, dtype=np.int64)
    estimator = LogisticRegression(
        C=float(config.inverse_regularization),
    ...
    if config.standardize_features:
        estimator_obj = make_pipeline(StandardScaler(), estimator)
    else:
        estimator_obj = estimator
    estimator_obj.fit(support_x, y)
    pred = np.asarray(estimator_obj.predict(query_x), dtype=np.int64)
    if config.standardize_features:
        fitted_logistic = estimator_obj.named_steps["logisticregression"]
    else:
        fitted_logistic = estimator
    coef = np.asarray(fitted_logistic.coef_, dtype=np.float64)
    intercept = np.asarray(fitted_logistic.intercept_, dtype=np.float64)
    diagnostics = {
...
def _plot_k_curve(overall_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.errorbar(
        ...
        linewidth=2,
        capsize=4,
        label="Logistic linear head",
    )
    ax.set_xlabel("K labeled support windows per activity")
    ax.set_ylabel("Macro F1")
    ax.set_title("Logistic Linear Head LOSO Evaluation")
...
def run(config: Config) -> dict[str, Any]:
    ...
    for fold in folds:
        ...
        fold_fp = config_fingerprint(
            {
                ...
                "baseline": "iterative logistic-regression fit of the final linear layer on support embeddings",
        ...
        for k in config.k_values:
            ...
            iterator = tqdm(
                range(episode_count),
                desc=f"{fold.fold_id} logistic-head k={k}",
                leave=False,
            )
            for trial in iterator:
                ...
                if k == 0:
                    ...
                    pred, diagnostics = _logistic_episode_predict(
                        model,
                        embeddings_by_window,
                        support_idx,
                        query_idx,
                ...
                row = {
                    ...
                    "backbone_source": "ce",
                    "backbone_checkpoint": str(ckpt_path),
                    "adaptation": "logistic_linear_head",
                    "embedding_space": "backbone",
                    "inverse_regularization": float(config.inverse_regularization),
                    ...
                    "num_iterations": int(diagnostics["num_iterations"]),
            ...
            print(
                f"[{fold.fold_id}] logistic-head k={k} "
    ...
    if subject_k_df.empty:
        raise RuntimeError("No logistic linear-head evaluation rows were produced.")
    ...
    for k, matrix in sorted(confusion_accumulator.items()):
        cm_path = eval_dir / f"confusion_logistic_linear_head_k{k}.png"
        save_confusion_matrix_plot(
            matrix,
            class_names(cfg),
            cm_path,
            title=f"Logistic Linear Head Confusion Matrix (K={k})",
        )
        confusion_paths.append(str(cm_path))

    summary = {
        "config": asdict(config),
        "baseline": "iterative logistic-regression fit of the final linear layer on labeled support embeddings",
