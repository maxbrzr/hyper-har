...
class Config:
    ...
    eval_stage_name: str = "19_closed_form_linear_head_support_loso"
...
@torch.no_grad()
def _closed_form_episode_predict(
    model: torch.nn.Module,
    ...
    if not support_indices:
        raise ValueError("Closed-form linear-head fit needs support embeddings.")
    ...
    feature_mean = torch.zeros(support_emb.shape[1], dtype=support_emb.dtype)
    feature_std = torch.ones(support_emb.shape[1], dtype=support_emb.dtype)
    ...
    if config.fit_bias:
        support_design = torch.cat(
            [support_emb, torch.ones(support_emb.shape[0], 1, dtype=support_emb.dtype)],
            dim=1,
        )
        query_design = torch.cat(
            [query_emb, torch.ones(query_emb.shape[0], 1, dtype=query_emb.dtype)],
            dim=1,
        )
    else:
        support_design = support_emb
        query_design = query_emb

    class_to_local = {
        int(activity_id): pos for pos, activity_id in enumerate(activity_ids)
    }
    local_y = torch.tensor(
        [class_to_local[int(label)] for label in support_y.tolist()],
        dtype=torch.long,
    )
    num_classes = int(len(activity_ids))
    targets = F.one_hot(local_y, num_classes=num_classes).to(dtype=support_emb.dtype)
    if float(config.label_smoothing) > 0:
        smooth = float(config.label_smoothing)
        targets = targets * (1.0 - smooth) + smooth / float(num_classes)

    xtx = support_design.T @ support_design
    penalty = torch.eye(xtx.shape[0], dtype=xtx.dtype)
    if config.fit_bias:
        penalty[-1, -1] = 0.0
    xty = support_design.T @ targets
    weights = torch.linalg.solve(xtx + float(config.ridge_lambda) * penalty, xty)
    logits = query_design @ weights
    local_pred = logits.argmax(dim=1).numpy()
    pred = np.asarray([int(x) for x in activity_ids], dtype=np.int64)[local_pred]
    diagnostics = {
        "head_weight_norm": float(
            weights[:-1].norm(p=2).item()
            if config.fit_bias
            else weights.norm(p=2).item()
        ),
        "head_bias_norm": float(
            weights[-1].norm(p=2).item() if config.fit_bias else 0.0
        ),
...
def _plot_k_curve(overall_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.errorbar(
        ...
        linewidth=2,
        capsize=4,
        label="Closed-form linear head",
    )
    ax.set_xlabel("K labeled support windows per activity")
    ax.set_ylabel("Macro F1")
    ax.set_title("Closed-Form Linear Head LOSO Evaluation")
...
def run(config: Config) -> dict[str, Any]:
    ...
    eval_stage_parts = [
        ...
        "ce_backbone",
        f"ridge_{float(config.ridge_lambda):g}".replace(".", "p"),
    ]
    if config.fit_bias:
        eval_stage_parts.append("bias")
    if config.standardize_features:
        eval_stage_parts.append("standardized")
    ...
    for fold in folds:
        ...
        fold_fp = config_fingerprint(
            {
                ...
                "baseline": "closed-form ridge fit of the final linear layer on support embeddings",
        ...
        for k in config.k_values:
            ...
            trial_head_weight_norms: list[float] = []
            ...
            iterator = tqdm(
                range(episode_count),
                desc=f"{fold.fold_id} closed-form-head k={k}",
                leave=False,
            )
            for trial in iterator:
                ...
                if k == 0:
                    ...
                    diagnostics = {
                        ...
                        "head_bias_norm": 0.0,
                        "feature_mean_norm": 0.0,
                        "feature_std_mean": 1.0,
                    ...
                    pred, diagnostics = _closed_form_episode_predict(
                        model,
                        embeddings_by_window,
                        support_idx,
                        query_idx,
                ...
                trial_head_weight_norms.append(float(diagnostics["head_weight_norm"]))
                ...
                row = {
                    ...
                    "backbone_source": "ce",
                    "backbone_checkpoint": str(ckpt_path),
                    "adaptation": "closed_form_linear_head",
                    "embedding_space": "backbone",
                    "ridge_lambda": float(config.ridge_lambda),
                    "fit_bias": bool(config.fit_bias),
                    "standardize_features": bool(config.standardize_features),
                    "label_smoothing": float(config.label_smoothing),
                    "head_weight_norm": float(diagnostics["head_weight_norm"]),
                    "head_bias_norm": float(diagnostics["head_bias_norm"]),
                    "feature_mean_norm": float(diagnostics["feature_mean_norm"]),
                    "feature_std_mean": float(diagnostics["feature_std_mean"]),
                    "support_embedding_norm_mean": float(
            ...
            head_weight_norm_mean, head_weight_norm_std, _ = mean_std_ci(
                trial_head_weight_norms
            )
            summary_row = {
                "test_subject_id": int(fold.test_subject_ids[0]),
                "k": int(k),
                ...
                "head_weight_norm_mean": head_weight_norm_mean,
                "head_weight_norm_std": head_weight_norm_std,
            }
            ...
            print(
                f"[{fold.fold_id}] closed-form-head k={k} "
    ...
    if subject_k_df.empty:
        raise RuntimeError("No closed-form linear-head evaluation rows were produced.")
    ...
    for k, matrix in sorted(confusion_accumulator.items()):
        cm_path = eval_dir / f"confusion_closed_form_linear_head_k{k}.png"
        save_confusion_matrix_plot(
            matrix,
            class_names(cfg),
            cm_path,
            title=f"Closed-Form Linear Head Confusion Matrix (K={k})",
        )
        confusion_paths.append(str(cm_path))

    summary = {
        "config": asdict(config),
        "baseline": "closed-form ridge fit of the final linear layer on labeled support embeddings",
