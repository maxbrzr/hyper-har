...
class Config:
    ...
    comparison_stage_name: str = "13_k_curve_mapem_vs_fixed_grid"
    ...
    map_em_color: str = "#7f7f7f"
    map_em_centered_color: str = "#1f77b4"
    fixed_color: str = "#d62728"
...
def _load_curve(
    dataset_dir: Path, prefix: str, max_k: int, prefer_contains: tuple[str, ...] = ()
) -> pd.DataFrame | None:
...
def run(config: Config) -> dict[str, Any]:
    ...
    for idx, dataset_id in enumerate(config.dataset_ids):
        ...
        uncentered_stage = next(
            (p for p in map_em_candidates if p != centered_stage),
            None,
        )
        ...
        if map_em_centered_df is None and map_em_uncentered_df is None:
            ...
            fallback_df = _load_curve(dataset_dir, config.map_em_prefix, config.max_k)
            map_em_uncentered_df = _prepend_k0_from_fixed(fallback_df, fixed_baseline)
        ...
        if map_em_centered_df is not None and not map_em_centered_df.empty:
            ax.plot(
                ...
                markersize=3.8,
                linestyle="-",
                color=config.map_em_centered_color,
    ...
    if unique:
        legend_order = ["MAP-EM centered", "MAP-EM", "Fixed Proto"]
    ...
    png_path = out_dir / "k_curve_mapem_vs_fixed_2x2.png"
    pdf_path = out_dir / "k_curve_mapem_vs_fixed_2x2.pdf"
    fig.savefig(png_path)
    fig.savefig(pdf_path)
    plt.close(fig)

    values_csv = out_dir / "k_curve_mapem_vs_fixed_values.csv"
