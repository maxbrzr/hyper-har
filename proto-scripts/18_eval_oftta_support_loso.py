...
class Config:
    ...
    batch_size: int = 256
    num_workers: int = 0
    edtn_decay: float = 0.3
    edtn_variance_floor: float = 1e-5
    support_capacity_per_class: int = 32
    include_classifier_weights: bool = True
    ...
    eval_stage_name: str = "18_oftta_support_loso"
...
def _bn_modules(model: torch.nn.Module) -> list[torch.nn.BatchNorm2d]:
    return [m for m in model.modules() if isinstance(m, torch.nn.BatchNorm2d)]


def _edtn_priors(num_layers: int, decay: float) -> list[float]:
    if float(decay) < 0:
        raise ValueError("edtn_decay must be >= 0.")
    return [float(1.0 - math.exp(-float(decay) * idx)) for idx in range(num_layers)]


def _batch_channel_stats(x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    if x.dim() != 4:
        raise ValueError(f"EDTN expects 4D BN input, got {tuple(x.shape)}.")
    dims = (0, 2, 3)
...
def _edtn_context(
    ...
    config: Config,
    mode: str,
    frozen_stats: dict[int, tuple[torch.Tensor, torch.Tensor]] | None = None,
) -> Iterator[dict[int, tuple[torch.Tensor, torch.Tensor]]]:
    modules = _bn_modules(model)
    priors = _edtn_priors(len(modules), config.edtn_decay)
    originals: dict[torch.nn.BatchNorm2d, Any] = {}
    mixed_stats: dict[int, tuple[torch.Tensor, torch.Tensor]] = {}

    def make_forward(
        layer_idx: int,
        module: torch.nn.BatchNorm2d,
    ) -> Any:
        def forward(self: torch.nn.BatchNorm2d, x: torch.Tensor) -> torch.Tensor:
            prior = priors[layer_idx]
            if mode == "estimate":
                batch_mean, batch_var = _batch_channel_stats(x)
                running_mean = self.running_mean.to(x.device, dtype=x.dtype)
                running_var = self.running_var.to(x.device, dtype=x.dtype)
                mean = prior * running_mean + (1.0 - prior) * batch_mean
                var = prior * running_var + (1.0 - prior) * batch_var
                var = var.clamp_min(float(config.edtn_variance_floor))
                ...
                raise ValueError("EDTN mode must be 'estimate' or 'frozen'.")
            return F.batch_norm(
                x,
                mean,
                var,
    ...
    for idx, module in enumerate(modules):
        ...
        module.forward = make_forward(idx, module)  # type: ignore[method-assign]
    try:
        yield mixed_stats
    finally:
...
def _stack_window_tensor(loader: Any, indices: Sequence[int]) -> torch.Tensor:
    if not indices:
        raise ValueError("Cannot stack an empty set of windows.")
    windows = [
        torch.from_numpy(sample_window_array(loader, int(idx))).float()
        for idx in indices
    ]
...
def _extract_vanilla_logits(
    ...
    return torch.cat(logits_list), torch.cat(labels), torch.cat(subjects)


@torch.no_grad()
def _encode_with_estimated_edtn(
    model: torch.nn.Module,
    x: torch.Tensor,
    device: torch.device,
    config: Config,
    ...
    model.eval()
    x = prepare_inputs(x).to(device).float()
    with _edtn_context(model, config, mode="estimate") as stats:
        emb = model.encode(x).cpu()
    return emb, {idx: (mean.cpu(), var.cpu()) for idx, (mean, var) in stats.items()}


@torch.no_grad()
def _encode_with_frozen_edtn(
    model: torch.nn.Module,
    x: torch.Tensor,
    device: torch.device,
    config: Config,
    frozen_stats: dict[int, tuple[torch.Tensor, torch.Tensor]],
    batch_size: int,
) -> torch.Tensor:
    model.eval()
...
def _oftta_episode_predict(
    ...
    if not support_indices:
        raise ValueError("OFTTA needs at least one unlabeled support embedding.")
    support_x = _stack_window_tensor(loader, support_indices)
    query_x = _stack_window_tensor(loader, query_indices)
    support_emb, frozen_stats = _encode_with_estimated_edtn(
        model,
        support_x,
        device,
        config,
    )
    query_emb = _encode_with_frozen_edtn(
        model,
        query_x,
        device,
        config,
    ...
    support_logits = model.classifier(support_emb.to(device)).cpu()
    support_logits_local = _restricted_logits(support_logits, activity_ids)
    support_entropy = _entropy_from_logits(support_logits_local)
    support_local_pred = support_logits_local.argmax(dim=1)
    activity_tensor = torch.tensor([int(x) for x in activity_ids], dtype=torch.long)
    support_pseudo_y = activity_tensor[support_local_pred]

    classifier_weight = model.classifier[0].weight.detach().cpu()
    prototypes: list[torch.Tensor] = []
    prototype_labels: list[int] = []
    selected_counts: dict[int, int] = {}
    for activity_id in activity_ids:
        vectors: list[torch.Tensor] = []
        if config.include_classifier_weights:
            vectors.append(
                F.normalize(
                    classifier_weight[int(activity_id)].view(1, -1),
                    dim=1,
                )[0]
            )
        cls_positions = torch.nonzero(
            support_pseudo_y == int(activity_id),
            as_tuple=False,
        ).view(-1)
        if cls_positions.numel() > 0:
            ent = support_entropy[cls_positions]
            order = torch.argsort(ent)
            keep = order[: int(config.support_capacity_per_class)]
            kept_positions = cls_positions[keep]
            cls_features = F.normalize(support_emb[kept_positions], p=2, dim=1)
            vectors.extend(cls_features)
            selected_counts[int(activity_id)] = int(kept_positions.numel())
        else:
            selected_counts[int(activity_id)] = 0
        if not vectors:
            continue
        proto = torch.stack(vectors, dim=0).mean(dim=0)
        prototypes.append(F.normalize(proto.view(1, -1), p=2, dim=1)[0])
        prototype_labels.append(int(activity_id))
    if not prototypes:
        raise ValueError("OFTTA could not build any prototypes.")
    prototype_tensor = torch.stack(prototypes, dim=0)
    prototype_label_tensor = torch.tensor(prototype_labels, dtype=torch.long)
    query_norm = F.normalize(query_emb, p=2, dim=1)
    proto_logits = torch.matmul(query_norm, prototype_tensor.T) / float(
        config.prototype_temperature
    )
    pred = prototype_label_tensor[proto_logits.argmax(dim=1)].numpy()
    diagnostics = {
        "num_prototypes": float(len(prototype_labels)),
        "support_selected_count_mean": float(np.mean(list(selected_counts.values()))),
...
def _plot_k_curve(overall_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.errorbar(
        ...
        linewidth=2,
        capsize=4,
        label="OFTTA support prototypes",
    )
    ax.set_xlabel("K unlabeled support windows per activity")
    ax.set_ylabel("Macro F1")
    ax.set_title("Offline OFTTA LOSO Evaluation")
...
def run(config: Config) -> dict[str, Any]:
    ...
    for fold in folds:
        ...
        fold_fp = config_fingerprint(
            {
                ...
                "paper": "Wang et al. Optimization-Free Test-Time Adaptation for Cross-Person Activity Recognition",
        ...
        test_logits, test_y, _test_subjects = _extract_vanilla_logits(
            model,
            test_loader,
            device,
        )
        ...
        for k in config.k_values:
            ...
            trial_entropy_means: list[float] = []
            ...
            for trial in iterator:
                ...
                if k == 0:
                    ...
                    diagnostics = {
                        "num_prototypes": 0.0,
                        "support_selected_count_mean": 0.0,
                        "support_entropy_mean": 0.0,
                        "support_entropy_min": 0.0,
                        "support_entropy_max": 0.0,
                        "support_embedding_norm_mean": 0.0,
                        "query_embedding_norm_mean": 0.0,
                    ...
                    pred, diagnostics = _oftta_episode_predict(
                        model,
                        loader,
                        support_idx,
                        query_idx,
                ...
                trial_entropy_means.append(float(diagnostics["support_entropy_mean"]))
                ...
                row = {
                    ...
                    "backbone_source": "ce",
                    "backbone_checkpoint": str(ckpt_path),
                    "adaptation": "oftta_offline_edtn_support_prototypes",
                    "embedding_space": "backbone",
                    "num_prototypes": int(diagnostics["num_prototypes"]),
                    "support_selected_count_mean": float(
                        diagnostics["support_selected_count_mean"]
                    ),
                    "support_entropy_mean": float(diagnostics["support_entropy_mean"]),
                    "support_entropy_min": float(diagnostics["support_entropy_min"]),
                    "support_entropy_max": float(diagnostics["support_entropy_max"]),
                    ...
                    "prototype_norm_mean": float(diagnostics["prototype_norm_mean"]),
                    "edtn_decay": float(config.edtn_decay),
                    "support_capacity_per_class": int(
                        config.support_capacity_per_class
                    ),
                    "include_classifier_weights": bool(
                        config.include_classifier_weights
                    ),
            ...
            entropy_mean, entropy_std, _ = mean_std_ci(trial_entropy_means)
            num_prototypes_mean, num_prototypes_std, _ = mean_std_ci(
                trial_num_prototypes
            )
            summary_row = {
                "test_subject_id": int(fold.test_subject_ids[0]),
                "k": int(k),
                ...
                "support_entropy_mean": entropy_mean,
                "support_entropy_std": entropy_std,
                "num_prototypes_mean": num_prototypes_mean,
                "num_prototypes_std": num_prototypes_std,
            }
    ...
    if subject_k_df.empty:
        raise RuntimeError("No OFTTA evaluation rows were produced.")
    ...
    for k, matrix in sorted(confusion_accumulator.items()):
        cm_path = eval_dir / f"confusion_oftta_support_k{k}.png"
        save_confusion_matrix_plot(
            matrix,
            class_names(cfg),
            cm_path,
            title=f"Offline OFTTA Confusion Matrix (K={k})",
        )
    ...
    summary = {
        ...
        "paper": {
            ...
            "arxiv": "2310.18562",
            "method": "offline EDTN support statistics plus entropy-ranked support prototypes",
        },
        ...
        "embedding_space": "backbone",
        "edtn_decay": float(config.edtn_decay),
        "support_capacity_per_class": int(config.support_capacity_per_class),
        "include_classifier_weights": bool(config.include_classifier_weights),
        "prototype_temperature": float(config.prototype_temperature),
